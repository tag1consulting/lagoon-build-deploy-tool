...
func TestGenerateDeploymentTemplate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "test22 - nginx-php ServiceValues toleration nodeselector with bad selector",
			args: args{
				buildValues: generator.BuildValues{
					Project:         "example-project",
					Environment:     "environment-name",
					...
					Services: []generator.ServiceValues{
						{
							...
							// Tolerate any node with a taint with key "gpu"
							Tolerate: "gpu",
							// invalid selector, should omit selector
							NodeSelector: "hardware:gpu:gibberish",
						},
						{
							Name:             "php",
							OverrideName:     "nginx",
			...
			want: "test-resources/deployment/result-nginx-php-toleration-2.yaml",
		},
	}
